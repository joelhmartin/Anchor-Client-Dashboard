...
async function classifyContent(prompt, transcript, message) {
...
async function fetchCtmCalls({ accountId, apiKey, apiSecret }, perPage = 100, maxPages = 5) {
...
export async function pullCallsFromCtm({ credentials, prompt = DEFAULT_AI_PROMPT, existingRows = [] }) {
  ...
  for (const raw of limited) {
    ...
    results.push({ call: callData, meta: { ...callData } });
  }
  return results;
}
